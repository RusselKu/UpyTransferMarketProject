...
<body class="h-screen w-screen flex flex-col overflow-hidden" id="main-body">
    ...
    <!-- Modal: Analytics -->
    <div id="modal-analytics" class="hidden fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
        <div class="bg-slate-900 w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-3xl p-6 flex flex-col gap-5 border border-slate-800 shadow-2xl">
            <div class="flex justify-between items-center border-b border-slate-800 pb-3">
                <h2 class="text-lg font-bold text-violet-400 flex items-center gap-2">
                    <span>📊 Analytics & SNA Insights Hub</span>
                    <span class="text-[10px] bg-violet-950 border border-violet-800/80 text-violet-300 font-bold px-2 py-0.5 rounded-full uppercase">Estadísticas de Red</span>
                </h2>
                <button onclick="closeAnalyticsModal()" class="text-slate-400 hover:text-white p-1 rounded-lg bg-slate-950">✕</button>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Chart 1: Degree Distribution (Power Law) -->
                <div class="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col gap-2 relative">
                    <span class="text-xs font-bold text-slate-300">1. Distribución de Grados (Scale-Free)</span>
                    <div class="h-48 w-full"><canvas id="chart-degree"></canvas></div>
                    <div class="bg-slate-900/60 p-2.5 rounded-xl border border-slate-800/50 mt-1 text-[10px] leading-relaxed text-slate-400">
                        <span class="text-violet-400 font-bold block mb-0.5">📌 Lectura del Gráfico:</span>
                        Demuestra la topología "Libre de Escala" (Ley de Potencias). Unos pocos clubes actúan como superhubs transaccionales, mientras que la inmensa mayoría tiene conexiones mínimas.
                    </div>
                </div>

                <!-- Chart 2: Top Centrality Metrics -->
                <div class="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col gap-2">
                    <span class="text-xs font-bold text-slate-300">2. Centralidad de Intermediación (Bridge Roles)</span>
                    <div class="h-48 w-full"><canvas id="chart-centrality"></canvas></div>
                    <div class="bg-slate-900/60 p-2.5 rounded-xl border border-slate-800/50 mt-1 text-[10px] leading-relaxed text-slate-400">
                        <span class="text-violet-400 font-bold block mb-0.5">📌 Lectura del Gráfico:</span>
                        Mide la intermediación (Betweenness). Identifica los clubes puente clave que canalizan el flujo de jugadores de un sector del mercado a otro.
                    </div>
                </div>

                <!-- Chart 3: Financial Spending -->
                <div class="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col gap-2">
                    <span class="text-xs font-bold text-slate-300">3. Volumen Financiero por Club (€ Millones)</span>
                    <div class="h-48 w-full"><canvas id="chart-financial"></canvas></div>
                    <div class="bg-slate-900/60 p-2.5 rounded-xl border border-slate-800/50 mt-1 text-[10px] leading-relaxed text-slate-400">
                        <span class="text-violet-400 font-bold block mb-0.5">📌 Lectura del Gráfico:</span>
                        Contraste directo entre lo gastado e ingresado. Las barras permiten auditar qué clubes financian la red (compradores netos) vs cuáles la abastecen (vendedores netos).
                    </div>
                </div>

                <!-- Chart 4: Assortativity Insights -->
                <div class="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col gap-2">
                    <span class="text-xs font-bold text-slate-300">4. Coeficiente de Homofilia por División</span>
                    <div class="h-48 w-full flex items-center justify-center flex-col gap-1.5 p-4 text-center">
                        <span class="text-3xl font-black text-emerald-400" id="stat-assortativity">-0.2175</span>
                        <span class="text-xs font-extrabold uppercase tracking-wider text-slate-300">Asortatividad por División / Liga</span>
                        <div class="max-w-md text-[10.5px] leading-relaxed text-slate-400 mt-1.5">
                            Al expandir los clubes extranjeros individuales en lugar de colapsarlos en ligas, el coeficiente es de <b>-0.2175 (Disasortativo)</b>. Esto demuestra cuantitativamente que el mercado español no es cerrado: los clubes prefieren transaccionar con divisiones diferentes y con el extranjero en lugar de cerrarse en transacciones domésticas.
                        </div>
                    </div>
                    <div class="bg-slate-900/60 p-2.5 rounded-xl border border-slate-800/50 text-[10px] leading-relaxed text-slate-400">
                        <span class="text-violet-400 font-bold block mb-0.5">📌 Lectura SNA:</span>
                        Un valor negativo confirma la disasortatividad (atracción entre distintos tipos de nodos), validando la dinámica de globalización e intercambio entre ligas en el mercado actual.
                    </div>
                </div>
            </div>
        </div>
